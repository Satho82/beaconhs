import {
  diaryRecurrenceCron,
  isOperationalTaskOverdue,
  type DiaryTemplateInput,
} from './diary-rules'
...
export async function listPropertyDiary(ctx: RequestContext, propertyId: string, now = new Date()) {
  ...
  const rows = await ctx.db((tx) =>
    tx
      .select({
        occurrence: operationalTaskOccurrences,
        schedule: operationalTaskSchedules,
        template: operationalTaskTemplates,
      })
      ...
      .orderBy(asc(operationalTaskOccurrences.dueAt)),
  )
  return rows.map((row) => ({
    ...row,
    overdue: isOperationalTaskOverdue(row.occurrence, now),
    overdueHours: Math.max(
      0,
      Math.floor((now.getTime() - row.occurrence.dueAt.getTime()) / 3600000),
    ),
  }))
}

export async function getDiaryTemplate(
  ctx: RequestContext,
  propertyId: string,
  scheduleId: string,
) {
  ...
  const [row] = await ctx.db((tx) =>
    tx
      .select({ schedule: operationalTaskSchedules, template: operationalTaskTemplates })
      ...
      .where(
        and(
          eq(operationalTaskSchedules.tenantId, ctx.tenantId),
          eq(operationalTaskSchedules.id, scheduleId),
          eq(operationalTaskSchedules.propertyId, propertyId),
          isNull(operationalTaskTemplates.deletedAt),
        ),
      )
...
export async function getDiaryOccurrenceDetail(
  ctx: RequestContext,
  propertyId: string,
  occurrenceId: string,
  now = new Date(),
  ...
  const [task] = await ctx.db((tx) =>
    tx
      ...
      .where(
        and(
          eq(operationalTaskOccurrences.tenantId, ctx.tenantId),
          eq(operationalTaskOccurrences.id, occurrenceId),
          eq(operationalTaskSchedules.propertyId, propertyId),
        ),
      )
      .limit(1),
  ...
  const { events, correctiveAction } = await ctx.db(async (tx) => ({
    events: await tx
      .select()
      .from(operationalTaskLifecycleEvents)
      .where(
        and(
          eq(operationalTaskLifecycleEvents.tenantId, ctx.tenantId),
          eq(operationalTaskLifecycleEvents.occurrenceId, occurrenceId),
        ),
      )
      .orderBy(asc(operationalTaskLifecycleEvents.processedAt)),
    correctiveAction:
      (
        await tx
          .select()
          ...
          .where(
            and(
              ...
              eq(correctiveActions.sourceEntityType, 'operational_task_occurrence'),
              eq(correctiveActions.sourceEntityId, occurrenceId),
            ),
          )
          .limit(1)
      )[0] ?? null,
  ...
  return {
    ...
    events,
    correctiveAction,
    overdue: isOperationalTaskOverdue(task.occurrence, now),
    overdueHours: Math.max(
      0,
      Math.floor((now.getTime() - task.occurrence.dueAt.getTime()) / 3600000),
    ),
  }
}
...
export async function createDiaryTemplate(
  ...
  const recurrence = {
    kind: input.recurrence,
    cron: diaryRecurrenceCron(input),
    dueOffsetMinutes: 0,
  }
  const result = await ctx.db(async (tx) => {
    const [template] = await tx
      .insert(operationalTaskTemplates)
      .values({ tenantId: ctx.tenantId, title, instructions: input.instructions?.trim() || null })
      .returning()
    ...
    const [schedule] = await tx
      ...
      .values({
        ...
        recurrence,
        startsAt: new Date(),
        assignedToTenantUserId: assigneeId,
      })
      .returning()
  ...
  await recordAudit(ctx, {
    entityType: 'operational_task_template',
    entityId: result.template.id,
    action: 'create',
    summary: `Created diary task template ${title}`,
    metadata: { propertyId, recurrence: input.recurrence },
  })
  return result
}

export async function updateDiaryTemplate(
  ctx: RequestContext,
  propertyId: string,
  scheduleId: string,
  input: DiaryTemplateInput,
  ...
  const result = await ctx.db(async (tx) => {
    ...
    const [template] = await tx
      .update(operationalTaskTemplates)
      .set({ title, instructions: input.instructions?.trim() || null })
      .where(
        and(
          eq(operationalTaskTemplates.tenantId, ctx.tenantId),
          eq(operationalTaskTemplates.id, schedule.templateId),
          isNull(operationalTaskTemplates.deletedAt),
        ),
      )
      .returning()
    ...
    const [updated] = await tx
      .update(operationalTaskSchedules)
      .set({ timezone, recurrence, assignedToTenantUserId: assigneeId })
      .where(
        and(
          eq(operationalTaskSchedules.tenantId, ctx.tenantId),
          eq(operationalTaskSchedules.id, scheduleId),
  ...
  await recordAudit(ctx, {
    ...
    entityId: result.template.id,
    action: 'update',
    summary: `Updated diary task template ${title}`,
    metadata: { propertyId, scheduleId },
  })
  return result
}

export async function setDiaryTemplateActive(
  ctx: RequestContext,
  propertyId: string,
  scheduleId: string,
  isActive: boolean,
  ...
  await gate(ctx, true)
  await propertyForTenant(ctx, propertyId)
  const [schedule] = await ctx.db((tx) =>
    tx
      .update(operationalTaskSchedules)
      .set({ isActive })
      .where(
  ...
  if (!schedule) throw new Error('Task schedule does not belong to this property.')
  await recordAudit(ctx, {
    entityType: 'operational_task_schedule',
    entityId: scheduleId,
    action: 'update',
    summary: `${isActive ? 'Enabled' : 'Disabled'} diary task schedule`,
    metadata: { propertyId },
  })
  return schedule
}

export async function completeDiaryTask(
  ctx: RequestContext,
  propertyId: string,
  occurrenceId: string,
  notes: string,
  ...
  const [owned] = await ctx.db((tx) =>
    tx
      .select({ id: operationalTaskOccurrences.id })
      .from(operationalTaskOccurrences)
      .innerJoin(
        operationalTaskSchedules,
        and(
    ...
    tx
      ...
      .where(
        and(
          ...
          eq(operationalTaskOccurrences.id, occurrenceId),
          or(
            eq(operationalTaskOccurrences.status, 'open'),
            eq(operationalTaskOccurrences.status, 'in_progress'),
            eq(operationalTaskOccurrences.status, 'overdue'),
          ),
        ),
      )
      .returning(),
  ...
  await recordAudit(ctx, {
    entityType: 'operational_task_occurrence',
    entityId: task.id,
    action: 'update',
    summary: 'Completed diary task',
    metadata: { propertyId, transition: 'completed' },
  })
  return task
}

export async function getOrCreateDiaryCorrectiveAction(
  ctx: RequestContext,
  propertyId: string,
  occurrenceId: string,
) {
  ...
  const result = await ctx.db(async (tx) => {
    ...
    const [row] = await tx
      ...
      .values({
        tenantId: ctx.tenantId,
        reference,
        title: `Overdue diary task: ${task.template.title}`,
        description: `Created from hospitality property ${propertyId}; task due ${task.occurrence.dueAt.toISOString()}.`,
        severity: 'high',
        status: 'open',
        source: 'other',
        sourceEntityType: 'operational_task_occurrence',
        sourceEntityId: occurrenceId,
        ...
        metadata: { propertyId, occurrenceId },
      })
      .returning()
  ...
  if (result.created)
    await recordAudit(ctx, {
      ...
      action: 'update',
      summary: `Created corrective action ${result.row.reference}`,
      metadata: { correctiveActionId: result.row.id, propertyId },
    })
  return result
}
