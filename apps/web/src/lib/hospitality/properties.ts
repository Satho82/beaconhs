...
export async function createBuilding(ctx: RequestContext, propertyId: string, name: string, code: string) { await gate(ctx, true); return ctx.db((tx) => tx.insert(hospitalityBuildings).values({ tenantId: ctx.tenantId, propertyId, name: clean(name,'Building name'), code: clean(code,'Building code') }).returning()) }
export async function createFloor(ctx: RequestContext, buildingId: string, name: string, code: string) { await gate(ctx, true); return ctx.db((tx) => tx.insert(hospitalityFloors).values({ tenantId: ctx.tenantId, buildingId, name: clean(name,'Floor name'), code: clean(code,'Floor code') }).returning()) }
export async function createRoom(ctx: RequestContext, floorId: string, code: string, name?: string, roomType?: string) { await gate(ctx, true); return ctx.db((tx) => tx.insert(hospitalityRooms).values({ tenantId: ctx.tenantId, floorId, code: clean(code,'Room code'), name: name?.trim() || null, roomType: roomType?.trim() || null }).returning()) }
