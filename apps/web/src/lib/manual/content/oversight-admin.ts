...
Use the single **Photo** field for every photo question. Under **Photo options**, choose whether the worker can add several photos, set the maximum, and turn on **Enable AI safety analysis** when you need it. Workers tap **Take photo or upload** to use the camera or choose pictures from their phone's photo library. Every photo supports the same pencil button for captions and freehand markup, the trash button for removal, and arrow buttons for reordering. Captions, markup, and order appear in generated PDFs and Flow photo fields. An AI review belongs to the exact attached photos, so replacing, adding, removing, or reordering a photo clears the old review.

For a **Diagram / sketch** field, build a reusable **Symbols** library so workers insert ready-made parts instead of drawing everything freehand. Open the field, tap **Add symbol**, draw the part on the capture canvas, tap **Save**, then give it a short name. Workers see each name above their canvas and tap to drop a copy they can move and edit. Symbols belong to the field, so draw whatever the form needs — crane parts for a lift plan, icons for a site map, or anything else. Workers with AI access also get **Draft with AI** on that canvas: they describe the diagram and the AI inserts an editable starting point they must review before saving.
