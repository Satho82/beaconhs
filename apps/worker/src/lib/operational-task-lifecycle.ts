...
export const OPERATIONAL_TASK_LIFECYCLE = {
  upcomingMinutes: 60,
  managerOverdueMinutes: 24 * 60,
  finalEscalationMinutes: 72 * 60,
} as const

type Stage = 'upcoming_reminder' | 'due_notification' | 'overdue_notification'

export function lifecycleStageForTask(
  ...
  if (task.dueAt.getTime() <= now.getTime() + OPERATIONAL_TASK_LIFECYCLE.upcomingMinutes * 60_000)
...
export async function processOperationalTaskLifecycle(now = new Date()) {
  const candidates = await withSuperAdmin(db, (tx) =>
    tx
      ...
      .innerJoin(
        tenantUsers,
        and(
          eq(tenantUsers.tenantId, operationalTaskOccurrences.tenantId),
          eq(tenantUsers.id, operationalTaskOccurrences.assignedToTenantUserId),
        ),
      )
      .innerJoin(
        tenantModuleEntitlements,
        and(
          eq(tenantModuleEntitlements.tenantId, operationalTaskOccurrences.tenantId),
      ...
      .where(
        and(
          ...
          isNull(operationalTaskOccurrences.completedAt),
          lt(
            operationalTaskOccurrences.dueAt,
            new Date(now.getTime() + OPERATIONAL_TASK_LIFECYCLE.upcomingMinutes * 60_000),
          ),
        ),
      )
      .limit(200),
  ...
  for (const row of candidates) {
    ...
    const [claimed] = await withSuperAdmin(db, (tx) =>
      tx
        ...
        .values({
          tenantId: row.occurrence.tenantId,
          occurrenceId: row.occurrence.id,
          stage,
          recipientUserId: row.assignee.userId,
          metadata: { propertyId: row.property.id },
        })
        .onConflictDoNothing()
        .returning({ id: operationalTaskLifecycleEvents.id }),
    )
    if (!claimed) continue
    await enqueueNotification(
      {
        tenantId: row.occurrence.tenantId,
        userIds: [row.assignee.userId],
        category: 'hospitality',
        type: `hospitality.task.${stage}`,
        title: `${stage === 'overdue_notification' ? 'Overdue' : 'Upcoming'} task: ${row.template.title}`,
        body: `${row.property.name} · due ${row.occurrence.dueAt.toISOString()}`,
        linkPath: `/hospitality/properties/${row.property.id}/diary`,
        data: { occurrenceId: row.occurrence.id, propertyId: row.property.id, stage },
      },
      { jobId: `hospitality-task|${row.occurrence.id}|${stage}|${row.assignee.userId}` },
    )
    processed += 1
  }
  ...
  const overdue = candidates.filter(
    (row) =>
      row.occurrence.dueAt.getTime() <=
      now.getTime() - OPERATIONAL_TASK_LIFECYCLE.managerOverdueMinutes * 60_000,
  )
  for (const row of overdue) {
    const managers = await withSuperAdmin(db, (tx) =>
      tx
        .selectDistinct({ userId: tenantUsers.userId })
        .from(tenantUsers)
        .innerJoin(
          roleAssignments,
          and(
        ...
        .where(
          and(
            eq(tenantUsers.tenantId, row.occurrence.tenantId),
            eq(tenantUsers.status, 'active'),
            sql`${roles.permissions} @> '["hospitality.manage"]'::jsonb`,
          ),
        ),
    )
      ...
      await enqueueNotification(
        {
          ...
          category: 'hospitality',
          type: 'hospitality.task.manager_notification',
          title: `Manager escalation: ${row.template.title}`,
          body: `${row.property.name} task remains overdue since ${row.occurrence.dueAt.toISOString()}`,
          linkPath: `/hospitality/properties/${row.property.id}/diary`,
          data: {
            occurrenceId: row.occurrence.id,
        ...
        { jobId: `hospitality-task|${row.occurrence.id}|manager|${manager.userId}` },
      )
      processed += 1
    }
    if (
      row.occurrence.dueAt.getTime() <=
      now.getTime() - OPERATIONAL_TASK_LIFECYCLE.finalEscalationMinutes * 60_000
    ) {
      const [claimed] = await withSuperAdmin(db, (tx) =>
        tx
          .insert(operationalTaskLifecycleEvents)
